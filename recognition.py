...
# Cosine Similarity: 1 is a perfect match. 
# A higher threshold is stricter.
COSINE_THRESHOLD = 0.8
# confidence threshold for MTCNN face detection
MIN_CONFIDENCE = 0.95
...
# Important for multi-face processing.
mtcnn = MTCNN(
    image_size=160,
    margin=0,
    keep_all=True,
...
def encode_known_faces(known_faces_dir, encodings_file):
    ...
    ** Supports multiple images per person and multiple faces per image. **
    A naming convention like 'person-name_description.jpg' is used.
    """
    ...
    for filename in os.listdir(known_faces_dir):
        ...
        # --- NEW: MODIFIED NAME EXTRACTION ---
        # extract name from filename (e.g., "john-doe_frontal.jpg" -> "john-doe")
        # allows multiple images for the same person by
        # splitting at the first hyphen or underscore.
        base_name = os.path.splitext(filename)[0]
        name = re.split(r'[_-]', base_name, maxsplit=1)[0]
        # --- END MODIFICATION ---
        ...
        # mtcnn() returns a batch of face tensors.
        faces = mtcnn(img_rgb)
        if faces is None:
            print(
                f"[WARNING] No face detected in {filename} for person '{name}'. Skipping."
            )
            continue
        ...
        for i, face_to_encode in enumerate(faces):
            with torch.no_grad():
                # embedding for the single face
                embedding = resnet(face_to_encode.unsqueeze(0).to(DEVICE))
                known_face_encodings.append(embedding.cpu().numpy()[0])
                # append the extracted, clean name
                known_face_names.append(name)
                print(
                    f"  > Encoded face {i+1}/{len(faces)} for: '{name}' from file: {filename}"
                )
...
def main(args):
    if not os.path.exists(KNOWN_FACES_DIR):
        ...
        print(
            "[INFO] Use a naming convention like 'person-name_description.jpg' (e.g., john-deo_frontal.jpg)"
        )
        return
    ...
    print(
        f"[INFO] Loaded {len(known_face_encodings)} known face encodings for {len(set(known_face_names))} people."
    )
    ...
    while True:
        ...
        if boxes is not None:
            for box, name in zip(boxes, names):
                ...
                label_y = box[1] - 15 if box[1] - 15 > 15 else box[1] + 15
                cv2.rectangle(
                    frame,
                    (box[0], label_y - label_size[1]),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Real-Time Face Recognition with FaceNet-PyTorch"
    )
    parser.add_argument(
